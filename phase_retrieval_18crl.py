...
#im=axs[1].imshow(rdata[0],extent=[x_mu.min(), x_mu.max(), x_mu.min(), x_mu.max()])
...
#im=axs[2].imshow(rdata[-1],extent=[x_mu_last.min(), x_mu_last.max(), x_mu_last.min(), x_mu_last.max()])
...
result_2 =calculate_average_cluster_height(recCTFPurePhase)
...
sd_CTF_homo= homoCTF(rads, wlen, dist, delta, beta, fx, fy, Rm, alpha)
...
ax3.text(1, 4.6, "Line 1", color='black', ha='center', va='center', fontsize=15)
...
ax4.text(1, 4.7, "Line 2", color='black', ha='center', va='center', fontsize=15)
ax4.set_ylim(4.45, 5.65)
