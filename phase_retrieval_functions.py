...
def calculate_matrix_height_difference(matrix):
    # Flatten the matrix for easier sorting
    flattened_matrix = matrix.flatten()
    
    # Define thresholds for gold pattern, substrate, and transition areas
    gold_threshold = np.percentile(flattened_matrix, 90)  # Top 10% values for gold
    substrate_threshold = np.percentile(flattened_matrix, 10)  # Bottom 10% values for substrate
    
    # Classify the data into gold, substrate, and transition areas
    gold_pattern = flattened_matrix[flattened_matrix >= gold_threshold]
    substrate = flattened_matrix[flattened_matrix <= substrate_threshold]
    
    # Calculate the mean of the gold pattern and substrate
    mean_gold = np.mean(gold_pattern)
    mean_substrate = np.mean(substrate)
    
    # Calculate the standard deviations of gold and substrate
    std_gold = np.std(gold_pattern)
    std_substrate = np.std(substrate)
    
    # Calculate the pooled standard deviation
    n_gold = len(gold_pattern)
    n_substrate = len(substrate)
    
    pooled_std = np.sqrt(((n_gold - 1) * std_gold**2 + (n_substrate - 1) * std_substrate**2) / (n_gold + n_substrate - 2))
    
    # Calculate the difference in mean absorption values between gold and substrate
    height_diff = mean_gold - mean_substrate
    
    return height_diff, pooled_std
